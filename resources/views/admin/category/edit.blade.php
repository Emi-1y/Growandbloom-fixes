    ...
    <div class="col-lg-8">
        <div class="admin-form-card">
            <div class="card-body">
                ...
                <form method="POST" action="{{ route('admin.category.update', $viewData['category']->getId()) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <textarea id="description" name="description" rows="4"
                                  class="form-control @error('description') is-invalid @enderror">{{ old('description', $viewData['category']->getDescription()) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="admin-form-actions">
                        <button type="submit" class="admin-btn-primary">
                            <i class="bi bi-check-lg" aria-hidden="true"></i> {{ __('category.update_button') }}
                        </button>
                        <a href="{{ route('admin.category.index') }}" class="admin-btn-secondary">
